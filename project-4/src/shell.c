	...
	while (1) {
		...
		/* Close shell */
			...
			if(argc == 2) {
				...
				/*Restrict which pid you can call kill in shell/windo*/
				if(n <= 2){
					shprintf("Can not kill a thread.\n");
					continue;
				}
				if(n == 5)
				{
					shprintf("Can not kill the keyboard.\n");
					continue;
				}
				
				kill(n);
				
				/* call on kill function is in scheduler*/
				int vl = kill(n);

				/*Print to shell if pid is real or not*/
				if (vl == -1) 
				{
					shprintf("Pid is not real. \n");
				}
				if(vl == 0)
				{
					shprintf("Process dead.\n");
				}
