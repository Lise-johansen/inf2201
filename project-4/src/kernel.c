...
#pragma GCC diagnostic ignored "-Wcast-function-type"
	...
	init_syscall(SYSCALL_KILL, (syscall_t)kill);
...
{
	static char *status[] = {"Running ", "Blocked ", "Sleeping",
				 "Exited  ", "Killed"};
	int i, j, base;
	pcb_t *p;
...
{
	/* Make a pointer that points to buf and make a intermidary buffeer*/
	char *tmp = (char *)buf;
	char tmp_buf[SECTOR_SIZE];

	/*Reads bootblock (512 byte block) into the intermidary buffer*/
	int read_0 = scsi_read(0, 1, tmp_buf);

	/*Assert if scsi() dose not return 0, and print error message*/
	ASSERT2((read_0 == 0), "Failed to read bootblock from disk");


	/* 
	 * Retrieve (sector 0 + 2 = process directory) and cast os size into an
	 * unsiged int
	 * Source for type casting: https://stackoverflow.com/questions/58566649/what-does-int-mean-in-c-in-this-context
	 */
	uint32_t os_size = *(uint32_t *)(tmp_buf + 2);

	/*Reads the process directory into the intermidary buffer*/
	int read_2 = scsi_read((os_size + 1), 1, tmp_buf);

	/*Assert if scsi() dose not return 0, and print erro message*/
	ASSERT2((read_2 == 0), "Failed to read directory from disk");

	/*Transfer data between intrmidary buffer and the tmp that points to the buf*/
	bcopy(tmp_buf, tmp, SECTOR_SIZE);
...
{
	/*
	 * Step 1: allocate memory for the process
	 */
	int *bace_addr = (int *)alloc_memory(STACK_SIZE + (SECTOR_SIZE * size));

	/*
	 * Step 2: read data from disk to bace address.
	 */

	int read_3 = scsi_read(location, size, bace_addr);
	ASSERT2((read_3 == 0), "Failed to read code/data from disk");

	/*
	 * Step 3: Create a process by calling create_process() function.
	 * Creat the process starting at bace address with size of process + the
	 * stack size. of the memory block allocated in step 1
	 */
	create_process(bace_addr, STACK_SIZE + (SECTOR_SIZE * size));
