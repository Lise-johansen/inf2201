...
int block_read(int block_num, void *address)
{
	...
	// int read_to = scsi_read_part(block_num, 1, (char *)address);
	int read_to = scsi_read(block_num, 1, (char *)address);
	// ASSERT2(read_to == 0, "read error");
	return read_to;
}
...
{
	// int written_to = scis_write_part(block_num, 0, BLOCK_SIZE, (char *)address);
	int written_to = scsi_write(block_num, 1, (char *)address);
	ASSERT2(written_to == 0, "write error");
