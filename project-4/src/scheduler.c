...
void scheduler(void) {
	...
	do {
		...
		case KILLED:
			...
			/*
			* Here in scheduler the next job is chosen, and the current running is removed from the ready queue.
			* First check if no more jobs, loop forever 
			*/
			if (current_running->next == current_running) {
				HALT("No more jobs.");
			}
			...
			/* Check again if the current running holds any locks*/
			if(current_running->use_lock == 0)
			{
				/* Go to next job*/
				current_running = current_running->next;

				/*free the currently running previous*/
				free_pcb(current_running->previous);
			}
			else
			{
				/* If current running hold onto a lock, go to next job*/
				current_running = current_running->next;
			}
...
int kill(int pid)
{
	/* Here in this function a process is mark as killed */
	enter_critical();

	/* Get the pid of the currently running process and set flag to 0*/
	int current_pid = getpid();
	int found_pid = 0;

	/* Make a tmp for currently running*/
	pcb_t *tmp = current_running;

	/* If the pid to the currently running is not equal to the pid from argv[1] terminal*/
	while(tmp != NULL && tmp->pid != pid)
	{	
		/* And if the pid is not real*/
		if(tmp->next->pid == current_pid)
		{
		...
		/* The pid is not the one so go to the next job*/
		tmp = tmp->next;
	}

	/* Return the value signaling that the pid is not real*/
	if (found_pid == 1)
	{
		/* Exit the critical section before returning*/
		leave_critical();

		/*Retrun -1 for no-existing pid*/
		return -1;
	}

	/*Check if the process with the givven pid id found (again) */
	if(tmp != NULL && tmp->pid == pid)
	{
		/* Mark currently running as with status killed*/
		tmp->status = KILLED;
	}
