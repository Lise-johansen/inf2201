...
{
	/*Keyboard uses a mbox, so need to initilaze it before the keyboard.*/
...
{
	/*Buffer space for m*/
	char s[CHAR_MSG_SIZE];
	msg_t *m = (msg_t *)s;

	enter_critical();

	/*Fetch the character from buffer and store in m*/
	int result = mbox_recv(mbox, m);
	leave_critical();

	/*Check if the result (the character from buffer) is valid, if not return -1*/
	if (result < 0) {
		return -1;
	}

	/*Transfer/ write the message (ASCII character) into c*/
	*c = (char *)m->body[0];
...
void putchar(struct character *c)
{
	...
	/*Check if there are available space in buffer, need acces to the space variable*/
	mbox_stat(mbox, &count, &space);

	/*Buffer space for m*/
	char s[CHAR_MSG_SIZE];
	msg_t *m = (msg_t *)s;

	/*Size of one char character is one byte*/
	m->size = 1;

	/*Transfer character into body*/
	m->body[0] = c->character;

	enter_critical();

	/* Check if space is full, so long space have enough place for message*/
	if (space >= CHAR_MSG_SIZE) {

		/*Insert character into the mbox buffe*/
		mbox_send(mbox, m);

	} else {

		/*
		 * If there is not enough space in the buffer just discard the message and return -1
		 */
		return -1;
	}
