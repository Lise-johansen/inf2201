...
typedef struct
{
	...
	int status;		/* a lock can be locked or unlocked */
} lock_t;
...
{
	pcb_t *waiting; /* waiting queue*/
	int value;		/* value representing the shared resources*/
...
{
	pcb_t *waiting; /* waiting queue */
	int count;		/* n is the number of threads that reaches the barrier*/
	int n_equal;	/* counter that increments (+1) when a thread reaches the barrier*/
} barrier_t;
